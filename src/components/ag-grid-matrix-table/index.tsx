  ...
  const handleCopyToClipboard = useCallback(async () => {
    try {
      ...
      const generateHtmlTable = (): string => {
        ...
        headerHtml += `<th border="1" style="padding: 8px; font-weight: bold; background-color: #e8f5e9;">Group</th>`;
        headerHtml += `<th border="1" style="padding: 8px; font-weight: bold; background-color: #e8f5e9;">Row Header</th>`;
        for (const colId of dataColumnOrder) {
          const colHeader = columnHeaders.find((c) => c.id === colId);
          const label = colHeader?.label ?? colId;
          headerHtml += `<th border="1" style="padding: 8px; font-weight: bold; text-align: center;">${label}</th>`;
        }
        headerHtml += "</tr>";
        ...
        for (const row of displayedRows) {
          ...
          rowHtml += `<td border="1" style="padding: 8px; text-align: left; background-color: #e8f5e9;">${groupValue}</td>`;
