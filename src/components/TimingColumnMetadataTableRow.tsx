...
export function TimingColumnMetadataTableRow({
  doeEntry,
  getScenarioDropdownConfig,
  handleDeleteDoe,
}: TimingColumnMetadataTableRowProps) {
  const dispatch = useAppDispatch();
  const { setNodeRef, style, attributes, listeners, isDragging } =
    useSortableDoERow(doeEntry.id);

  const [editingId, setEditingId] = useState<string | null>(null);
  const [editingValue, setEditingValue] = useState<string>("");

  const handleRenameStart = useCallback(() => {
    setEditingId(doeEntry.id);
    setEditingValue(doeEntry.label);
  }, [doeEntry.id, doeEntry.label]);
  ...
  return (
    <TableRow
      ref={setNodeRef}
      style={style}
      data-testid={`doe-row-${doeEntry.id}`}
    ...
    >
      <TableCell className="w-[40px] p-0 text-center">
        <Button
          ...
          data-testid={`doe-drag-handle-${doeEntry.id}`}
          {...attributes}
          {...listeners}
        >
      ...
      <TableCell className="font-medium w-[100px]">
        {editingId === doeEntry.id ? (
          <input
            ...
            data-testid={`doe-rename-input-${doeEntry.id}`}
          />
        ) : (
          <div className="flex items-center gap-1 group">
            <span data-testid={`doe-name-${doeEntry.id}`}>{doeEntry.label}</span>
            <Button
              variant="ghost"
              size="icon"
      ...
      <TableCell className="w-[80px]">{doeEntry.PROJECT_NAME || "-"}</TableCell>
      <TableCell className="w-[95px]">{doeEntry.BLOCK || "-"}</TableCell>
      <TableCell className="w-[120px]">{doeEntry.NET_VER || "-"}</TableCell>
      <TableCell className="w-[170px] truncate">
        {doeEntry.REVISION || "-"}
      </TableCell>
      <TableCell className="w-[105px]">{doeEntry.ECO_NUM || "-"}</TableCell>
      <TableCell>
